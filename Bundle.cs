...
class Bundle
{
    ...
    public Bundle(string path)
    {
        ...
        {
            var info = (NSDictionary)PropertyListParser.Parse(infoPlistPath);
            if (info.TryGetValue("CFBundleExecutable", out var temp) && temp is NSString bundleExecutable)
            {
    ...
    public void AddResourceRules(ResourceBuilder builder, bool useV2Rules = true)
    {
        ...
        if (useV2Rules)
        {
            ...
            builder.AddRule(new ResourceRule("^embedded\\.provisionprofile$") { IsOmitted = true, Weight = 20});
            builder.AddRule(new ResourceRule("^PkgInfo$") { IsOmitted = true, Weight = 20});

            // Include version.plist
